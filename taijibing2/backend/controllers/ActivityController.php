...
class ActivityController extends BaseController{
    ...
    public function actionAppUserActivity(){
        ...
        return $this->renderPartial('app-user-activity',['activity_id'=>$activity_id]);
    }
    ...
    public function actionSetDefaultActivity(){
        ...
        try{
            ...
            $data=ActiveRecord::findBySql("select Id from activity where State=1 and EndTime > NOW()")->asArray()->one();
            if($data){
                //将已有默认活动截至到当前，状态修改成非默认获得
                $re=\Yii::$app->db->createCommand("update activity set State=0,UpdateTime='$now',EndTime='$now' where State=1 and EndTime > NOW()")->execute();
    ...
    public function actionDelUser(){
        ...
        try{
            ...
            //修改用户参加的活动id
            $re=\Yii::$app->db->createCommand("update user_info set ActivityId=0 where Id in ($UserId_str)")->execute();
            if(!$re){
    ...
    public function actionGetIndexData(){
        ...
        select activity.Id,activity.Title,activity.StartTime,activity.EndTime,activity.State,
