...
<div class="wrapper wrapper-content">
  ...
  <li style="position:relative"><a href="javascript:void(0)" data-magnify="gallery" data-group="g1" data-src="http://www.taijibing.cn/static/upload/20181029163437-2810020214.jpg" data-caption="临时补拍" > <img src="http://www.taijibing.cn/static/upload/20181029163437-2810020214.jpg" style="width:200px"> <p>临时补拍</p></a> </li>
...
<!--            <span style="padding-left:10px;"><label>手机号:</label><input type="text" placeholder="请输入手机号" value="--><?//=$mobile?><!--" name="mobile"/></span>-->
        ...
        <table class="table table-hover" style="position: relative;margin-top: 15px;">
            <thead>
                ...
                foreach($model as $key=>$val)
                {
                    ...
                    $str.= "<tr ".(in_array($val['DevNo'],$users_of_init)?"date = '1'":'')."".(($val['IsActive']==0&&!in_array($val['DevNo'],$users_of_init))?"style='color: red;'":'').">
                          ...
                          <label  for='onten".$no."'></label>
                           ...
                           if($ImageState==1){
                                 $str.=" <a   onclick='picture(\"{$val['Image']},{$val['TempImage']},{$val['DevNo']},{$val['ImageState']},{$val['Name']}\")'>待确认</a> ";
                            }else if($ImageState==2){
                                  $str.=" <a   style='color:#00ff08'  onclick='picture(\"{$val['Image']},{$val['TempImage']},{$val['DevNo']},{$val['ImageState']},{$val['Name']}\")'>已通过</a> ";
                            }else if($ImageState==3){
                                  $str.=" <a   style='color:red'   onclick='picture(\"{$val['Image']},{$val['TempImage']},{$val['DevNo']},{$val['ImageState']},{$val['Name']},{$val['ImageErrorReason']}\")'>未通过</a> ";
                            }else{
                               $str.=" 未上传 ";
                                   // $str.=" <a   onclick='picture(\"{$val['Image']},{$val['TempImage']},{$val['DevNo']},{$val['ImageState']},{$val['Name']}\")'>已上传待确认</a> ";
                            }
                            $str.= " </td> </tr>";
                            // $str.=" <a>".(($val["ImageState"]==1)?'待确认':'没有上传')."</a> ";
                            //<a class='del' id='".$val["Id"]."'  href='javascript:void(0);'>删除</a></td>
                            //href='./?r=customer/untie&CodeNumber={$val['CodeNumber']}&devno={$val['DevNo']}'
                            // onclick="refund(\''+item.OutTradeNo+'\',\''+item.TransactionId+'\')"

                             // ".$val["IsUse"]==1?'selected'." 
                    $no++;
                }
                echo $str;
            }else{
    ...
    <script type="text/javascript" src="/static/js/index.js?v=1.23"></script>
