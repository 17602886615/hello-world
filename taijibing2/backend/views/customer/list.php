...
<div class="wrapper wrapper-content">
        ...
        <table class="table table-hover" style="margin-top: 15px;">
            ...
            <?= $role_id==1?"<th  style='width: 10%'><a id='sort' hreg=''>登记时间</a></th>":''?>
             ...
             <th style="width: 8%">推荐人</th>
              <th style="width: 8%">营销活动</th>
               <th style="width: 8%">赏金</th>
                ...
                foreach($model as $key=>$val)
                {
                    $str.= "<tr ".(in_array($val['DevNo'],$users_of_init)?"date = '1'":'')."".(($val['IsActive']==0&&!in_array($val['DevNo'],$users_of_init))?"style='color: red;'":'').">
                          ...
                          <label  for='onten".$no."'></label>
                            ...
                            <td>".$val["RecommendUserTel"]."</td>
                            <td>".($val["Title"]?$val["Title"]:$default_activity)."</td>
                            <td><a href='./?r=customer/money-detail&UserId=".$val["Id"]."'>".$val["Money"]."</a></td>
    ...
    <script>
      if(role_id!=1){
         $("#version,.state").css('display','none');
         $("table label").remove()
       }
            ...
            // $(this).parent().parent().line();
             ...
             console.log($(this).offset().top)
                    ...
                    var w = $(this).width();//3、获得对应行的宽度 
                     $(this).after("<div style='outline:#BABABF solid 1px; position:absolute; left:" + l + "px;top:" + t + "px;width:" + w + "px;z-index:1'></div>");//4
