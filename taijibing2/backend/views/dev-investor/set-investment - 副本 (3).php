...
$(document).on('click','#ghghgh',function(){ 
    dddd()
}); 
    ...
    var itemStr=' <div class="item" id="item'+currentIndex+'" itemid="100">'+
          ...
          $.get('./?r=dev-investor/get-devbrand',function(data){
             ...
             $(data).each(function(i,v){
              ...
              goodList.devbrand.append(html);
              goodList.devbrand.chosen({no_results_text: "没有找到"}); //初始化chosen
              goodList.devname.chosen({no_results_text: "没有找到"}); //初始化chosen
              goodList.devfactory.chosen({no_results_text: "没有找到"}); //初始化chosen
              goodList.province.chosen({no_results_text: "没有找到"}); //初始化chosen
              goodList.city.chosen({no_results_text: "没有找到"}); //初始化chosen
          ...
          if(brand_id){
            ...
            $.get('./?r=dev-investor/get-dev',{'devbrand_id': brand_id},function(data){
                ...
                if(data!=''){
                    ...
                    goodList.devname.trigger("chosen:updated");
                }else{
                    var html = "<option value=''>设备名称</option>";
...
//            console.log(html);
            ...
            goodList.devfactory.trigger("chosen:updated");
            //console.log(data);
        });
                //选中省
        if(address!=''){
            ...
            $(html).appendTo('#province'+currentIndex);
              goodList.province.trigger("chosen:updated");
        }
       //选中市
        ...
        if(province != ''){
            if(province=='全国'){
                ...
                goodList.city.trigger("chosen:updated");
            }else{
                //获取当前省对应的市 数据
