            ...
            <h3>创建商品</h3>
                <form action="<?=$goods->id?'./index.php?r=goods/edit-goods':'./index.php?r=goods/add-goods'?>" method="post" accept-charset="utf-8" class="form-horizontal">
                    ...
                    <div class='row'>
                            ...
                            <div class='col-xs-1 col-md-offset-1'><p style="line-height: 30px;">商品二级分类</p> </div>
                            <div class='col-xs-3'> 
                   ...
                   <div class='row' id='volumeGoods' style="display: none"> 
                    ...
                    <div class='row'  id='tonghua' style="display: none">
                            <div class='col-xs-1'><p style="line-height: 30px;">使用卡片厂家：</p> </div>
                            <div class='col-xs-3'> 
                            ...
                            <div class='col-xs-3'> 
                                 <input type="text" name="Goods[type]" value="<?=$goods->type?$goods->type:''?>" style="width:100%;" />
                            </div>
...
$('.addpGoods').click(function(){
    ...
    if(type1.length){
        $("#type1").empty()
          $("#type1").append("<option value>请选择</option>");
       ...
       //  $('.type1').trigger("liszt:updated");
       //  $('.type1').chosen({
       //        no_results_text: "没有找到结果！",//搜索无结果时显示的提示
       //        placeholder_text_single:"无"
       //     });
         
       // $("#type2").chosen({no_results_text: "没有找到",disable_search: false}); //初始化chosen
       // $('.type2').trigger("liszt:updated");
    }
...
function get_brands(Pid){
      ...
      $.get('./index.php?r=goods/get-brands&category1_id='+Pid, function(data) {
          ...
          $("#GoodsBrand_id").empty()
           $("#GoodsBrand_id").append("<option value>请选择</option>");
              for(var i=0;i<obj.brands.length;i++){
                var item = obj.brands[i];
     
                   $("#GoodsBrand_id").append('<option value="'+item.BrandNo+'">'+item.BrandName+'</option>'); 
                         // $("#type2").trigger("chosen:updated");  
               }
              var GoodsBrand_id  = $("#GoodsBrand_id").attr('data');
               $("#GoodsBrand_id").val(GoodsBrand_id)
