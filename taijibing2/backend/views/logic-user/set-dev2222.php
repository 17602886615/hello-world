...
<div style="position: absolute;right: -300px;top:10px;">
                    <a href="javascript:addGoodType();"  class='addition'><button type="button" class="btn btn-danger" style="    width: inherit;">添加条件</button></a>
                    <a href="javascript:void(0)" class='amend'  UId =''  style='display:none'><button type="button" class="btn btn-danger" style="    width: inherit;">确认修改</button></a>
...
var htmlInput = '<div class="main-title" style="padding:20px 0;"><img src="/static/images3/caption2.png" alt=""><span class="font-size-S" style="margin-left:15px;"> 请手动输入预设值</span> </div><input type="number" name=""  class="dvCBsrmb" value="" placeholder="请输入金额" style="background: #ebeef3;color:#000;border-radius:5px"/ >  &nbsp;元';
...
// window.location.reload();
    ...
    $.get('./?r=logic-user/add-use-type',{'use_type':''+prnb+'元','agent_id':id},function(data){ 
            if(data.state<0){
                console.log(data)
                return;
            }
     ...
     var html = '<div class="listBs"><input type="checkbox"  checked="checked" name="use_type" value='+data.code+'  id="state'+dvCBsIndex+'" class="state" / >'
        ...
        html+='<span> '+prnb+'&nbsp;元</span> <div class="Bgclose" style="display:'+displayDate+'"><i class="fa fa-times" aria-hidden="true"></i> </div><div>'
            //window.location.reload();
           $("#dvCBs").append(html)
...
// var   oiuy = datasd({'province':province,'city':city,'area':area},'./?r=logic-user/get-devinvestor');
// console.log(oiuy)
// console.log(baseGood)
layer.closeAll()
// console.log(baseGood)
   ...
   var facilityDate= $(".compilefacility").attr('date');
    ...
    displayDate='inline-block'
      ...
      // console.log(displayDate)
        ...
        // console.log(baseGood[index].id)

       var html = '<p  class="itemLi"  id="item_'+index+'" onjId='+baseGood[index].id+'> <span class="itemNUm">'+currentIndex+' </span>   <span class="iteminvestor" dataval="'+baseGood[index].investor_id+'">'+baseGood[index].Name+'</span><span class="itembrand"  dataval="'+baseGood[index].brand_id+'">'+baseGood[index].BrandName+'</span><span class="itemname"  dataval="'+baseGood[index].goods_id+'">'+baseGood[index].GoodsName+'</span><span class="itemclose" style="display:'+displayDate+';"><a href="javascript:void(0)"><span class="redact" date =1 style="min-width:50px;">编辑</span> </a><span style="min-width:50px;"  class="itemBgclose"> <img src="/static/images3/guanbi.png" alt=""></span></span> </p>';
