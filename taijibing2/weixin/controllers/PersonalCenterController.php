...
and send_water_log.AgentId=$AgentId and send_water_log.State=2")->asArray()->all();
    ...
    public function actionSaveRecommend(){
        ...
        //保存推荐用户即将获得的奖金记录
        $sql2="insert into user_recommend_log (`UserId`,`ActivityId`,`State`,`FirstMoney`,`DrinkMoney`,`Amount`,`RowTime`,`LoginUserId`)
        values ('$UserId',{$activity_info['Id']},0,{$activity_info['FirstMoney']},{$activity_info['DrinkMoney']},1,'$now','$login_user_id')";
        $transaction=Yii::$app->db->beginTransaction();
        try{
            //保存推荐的用户
    ...
    public function actionGetMyMoneyLog(){
        ...
        $Money=yii\db\ActiveRecord::findBySql("select Money from user_info where Id='$UserId'")->asArray()->one();
        if(!$Money){
            return json_encode(['state'=>-1,'msg'=>'没找到登陆用户']);
        }
        ...
        where user_recommend_log.LoginUserId='$UserId'
        union
        ...
        $datas=['data'=>$data,'money'=>$Money['Money']];
